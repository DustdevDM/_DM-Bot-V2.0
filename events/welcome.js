    ...
    user.guild.createChannel(user.displayName, "text", [
        ...
        channel.setParent(welcomechannel)
           ...
           user.guild.channels.get(ruleschan.channel).fetchMessage(ruleschan.msg).then(m => rules = m.content.replace("Discord Nutzervereinbarung", "[Discord Nutzervereinbarung](https://discordapp.com/terms)"))
            ...
            twitch_bot.say(user.user.tag + " ist gerade dem _DM Crew Discord Server beigetreten.        Du willst auch joinen? --> _discord")
        })
}
    //Rejoined Member
    else {     
            twitch_bot.say(user.user.tag + " ist gerade erneut dem _DM Crew Discord Server beigetreten.         Du willst auch joinen? --> _discord")
...
client.on("guildMemberRemove", (user) => {
    ...
    twitch_bot.say(user.user.tag + " hat den _DM Crew Discord Server verlassen")
